...
- Topic: 이벤트를 추적하거나 쿼리할 때 사용되는 요소로 데이터베이스의 인덱스에 해당하는 개념입니다. 하나의 이벤트에 토픽은 한 개 이상 존재할 수 있어며, 첫 번째 토픽은 특별히 이벤트의 호출 시그니처(ex. `Transfer(address,address,uint256)`)를 `keccak256` 해시 함수로 해시한 Hex rkqt dlqslek. (ex. `0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef`). 나머지 토픽들은 디벤트 내 각각 인자들 중 `indexed`가 붙은 인자들의 값입니다.
